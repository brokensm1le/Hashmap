...
public:
    ...
    HashMap(Hash hasher = Hash())
            : table_size_(defaultSize_), elem_count(0), hasher_(hasher), table_(defaultSize_) {}
...
template<class KeyType, class ValueType, class Hash>
void HashMap<KeyType, ValueType,
        Hash>::insert(const KeyValuePair& key_value_pair) {
...
void HashMap<KeyType, ValueType, Hash>::rebuild() {
    ...
    for (iterator it = elements_.begin(); it != elements_.end(); ++it) {
        size_t hash = hasher_(it->first) % (table_size_);
